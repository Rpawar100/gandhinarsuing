...
class Cn_opd_appointment extends MY_Controller {
...
public function get_pending_appointment_count() {
    $this->db->where('appointment_status', 'Pending');
    $this->db->from('appointment');
    $count = $this->db->count_all_results();
    return $count;
}

public function get_discharge_count() {
  ...
  public function index(){   
    ...
    $id = $this->session_validate();

    $data['inprocess_count'] = $this->get_inprocess_appointment_count();
    $data['pending_count'] = $this->get_pending_appointment_count();


    $data['inprocess_count'] = $this->get_inprocess_appointment_count();
     $query = $this->db->query("
                        SELECT COUNT(*) AS total_opd_appointments
                        FROM appointment
    ...
    $data['total_opd_new_appointments'] = $result->total_opd_appointments;


      $query = $this->db->query("
            SELECT COUNT(*) AS total_completed_appointments
            FROM appointment
            WHERE appointment_section_id = 1
    ...
    $this->load->view('vw_opd_appointment',$data);
  }
  ...
  public function opd_appointment_list() {
    $id = $this->input->get('id');

    if ($id != "") {
        // Your code for handling $id is not provided
    } else {
        $this->session_validate();
        ...
        $result_query = "SELECT
                        ...
                        from (
                            SELECT
                                ...
                                case 
                                    when amount is NULL then concat('<label appoint_id=\"',appointment_id,'\" appoint_id=\"',appointment_id,'\" style=\"font-size:initial;font-weight:bolder; color:orange;\">0.00</label>')
                                    else concat('<label class=\"view_services\" appoint_id=\"',appointment_id,'\" style=\"font-size:initial;font-weight:bolder; color:orange;\">',amount,'</label>')
                                end as total_amount,
                                '-' as print_label,
                                Case when appointment_received_amount=0.00 and appointment_pay_status='Paid' then 
                                    ...
                                    else concat('<label class=\"view_received_amount\" appoint_id=\"',appointment_id,'\"  style=\"font-size:initial;font-weight:bolder; color:green;\">',appointment_received_amount,'</label>')
                                end as received_amt,
                            ...
                            from appointment
                            left join patient on appointment_patient_id=P_id
                            left join department on appointment_department_id=department_id
                            left join user on appointment_doctor_id=user_id
                            left join patient_category on appointment_patient_category_id=patient_category_id
                            left join patient_company on appointment_patient_company_id=patient_company_id
                            left join (SELECT ASA_appointment_id, sum(ASA_service_amount) as amount 
                                       FROM appointment_services_assign 
                                       WHERE ASA_status IS NULL or ASA_status='Pending' 
                                       GROUP BY ASA_appointment_id) as amount_data on ASA_appointment_id = appointment_id
                            where appointment_section_id='1' 
  ...
  //           where appointment_section_id='1' and date_format(appointment_timestamp,'%Y-%m-%d') between '".$from_date."' and '".$to_date."'
  //         ) result_data,(SELECT @sr_no:= 0) AS a
  //         where appointment_no!=''
  //         ".$searchQuery."
  //         order by appointment_no desc
  ...
  public function opd_appointment_action(){ 
    $this->session_validate();
    if (!empty($_POST)) {
  ...
  public function opd_appointment_bill()
  {
    ...
    $services_query="SELECT
          ...
          where ASA_appointment_id='".$rid."' AND (ASA_status IS NULL or ASA_status='Pending')";
  ...
  public function get_unpaid_services(){

    $appoint_id=$this->input->post('appoint_id');

    $query="SELECT  ASA_id as id
            ,services_id as sid
            ,services_name as sname
            ,ASA_service_amount as amount 
            from appointment_services_assign
            join services on ASA_services_id=services_id 
            where ASA_appointment_id='".$appoint_id."' and ASA_payment_status NOT IN('Paid','0')";
