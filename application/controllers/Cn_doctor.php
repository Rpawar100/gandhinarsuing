...
class Cn_doctor extends MY_Controller {
	...
	public function user_action(){	
		...
		if (!empty($_POST)) {
			...
			$password = !empty($_POST['mobile_number']) ? $_POST['mobile_number'] : '';
	...
	public function user_by_id(){
		...
		if(!empty($_POST['user_id'])){
			...
			$data['record']=$this->db->query("	SELECT 
														...
														,user_aadhaar_number as aadhaar_number
														,user_pan_number as pan_number
														,user_aadhaar_front_url as aadhaar_front_photo
														,user_aadhaar_back_url as aadhaar_back_photo
														,user_pan_url as other_doc_photo
												FROM user
													join designation 
														on user_designation_id=designation_id
	...
	public function doctor_by_department_id(){
		...
		if(!empty($_POST['d_id'])){
			...
			$data['record']=$this->db->query("	SELECT 
												...
												where UDA_department_id='".$d_id."' and UCASE(user_designation_name) like '%DOCTOR%'and UDA_status='1'")->result_array();
			$this->json_output($data);
		}
	}
