...
class Cn_radiologist extends MY_Controller {
	...
	public function index(){   
		...
		$this->load->view('vw_radiologist');
	}

	public function radiologist_appointment_list(){
		$this->session_validate();
		$draw = $_POST['draw'];
		$start = $_POST['start'];
		$length = $_POST['length']; 
		...
		$result_query="	SELECT
			...
			from (
				SELECT 
						...
						,services_name
						,case 
						when appointment_status='Pending' then concat('<label class=\"change_report_status\" ASA_id=\"',ASA_id,'\" report_status=\"',appointment_status,'\" style=\"font-size:initial;font-weight:bolder; color:red;\">',appointment_status,'</label>')
					...
					,case 
						when ASA_service_report='' or ASA_service_report is null then concat('<i class=\"fa fa-eye generate_report\" style=\"font-size:initial;font-weight:bolder; color:black;\"  appointment_id=\"',appointment_id,'\"  data_url=\"https://aqdsoft.in/generate-lis-report/',ASA_id,'\" pdf_url=\"https://aqdsoft.in/generate-lis-report/',ASA_id,'?action=PDF\"></i>')
						else concat('<i class=\"fa fa-eye generate_report\" style=\"font-size:initial;font-weight:bolder; color:black;\"  appointment_id=\"',appointment_id,'\"  data_url=\"https://aqdsoft.in/generate-lis-report/',ASA_id,'\" pdf_url=\"https://aqdsoft.in/generate-lis-report/',ASA_id,'?action=PDF\"></i>')
	...
	public function change_report_status()
	{
		...
		if (!empty($_POST)) 
		{
			...
			{
				$this->db->query("UPDATE appointment set appointment_status='Completed',user_report_approved_timestamp=CURRENT_TIMESTAMP where user_id='".$user_id."'");

				$data['swall']=array(
						'title'=>'Done!',
						'text'=>'<b>Report Status Has been Updated to Verified..!</b>',
						'type'=>'success',
					);
				$this->json_output($data);

			}elseif ($report_status=='Rejected' && !empty($reason)) {

				$this->db->query("UPDATE appointment set appointment_status='Rejected',user_report_reject_comment= '".$reason."' where user_id='".$user_id."'");

				$data['swall']=array(
						'title'=>'Done!',
						'text'=>'<b>Report Status Has been Updated to Rejected..!</b>',
						'type'=>'success',
					);
				$this->json_output($data);
