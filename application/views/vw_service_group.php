  ...
  <div class="row">
    <div class="col-lg-4">
      <div class="ibox-title"  style="padding-top: 10px !important;">
        <h3 class="record_form_title">Add Service Group</h3>
      </div>
      <div class="ibox-content">
        <form method="post" enctype="multipart/form-data" id="record_form">
          <div class="row" style="margin: 0px;">
            <label class="form-control col-lg-5" style="border:0px;">Service Group Name<a style="color: red">*</a><b class="pull-right">:</b></label>
            <div class="col-lg-7">
                <input class="form-control" type="text" name="sg_name" id="sg_name" maxlength="50" required="" >
                <input class="form-control" type="hidden" name="sg_id" id="sg_id">
            </div>
          </div><hr>
          <div class="row" style="margin: 0px;">
...
<script type="text/javascript">
  $('#service_group_tab').addClass('active');
  get_list();
  ...
  function get_list(){
    ...
    var table=$('#datatable_list').DataTable({
      ...
      'stateSave': true,
      'ajax': {
        'url':'<?=base_url()?>service-group-list',
      },
      'columns': [
        {title: "Sr.No.",data:"sr_no",mSearch:false},
        {title: "Service Group Name",data:"name",mSearch:true},
        {title: "Services Count",data:"count",mSearch:false},
  ...
  function reset_form(){
    $('.record_form_title').html('Add Service Group');
    $('#sg_name').val('');
    $('#sg_id').val('');
  }
  ...
  $('#record_form').on('submit',function(event){
    ...
    if(form.valid()==true){
      $.ajax({
        url:'<?=base_url()?>service-group-action',
        method:'post',
        data:formData,
        cache: false,
        contentType: false,
  ...
  $(document).on('click','.edit_record',function () {
    ...
    if (sg_id) {
      $.ajax({
        url:'<?=base_url()?>service-group-by-id',
        method:'post',
        data:{sg_id:sg_id},
        async: false,
        success:function (data) {
          if(typeof(data)=='object'){
            $('.record_form_title').html('Edit Service Group');
            $('#sg_name').val(data['record']['name']);
            $('#sg_id').val(sg_id);
          }else{
            location.reload();
          }
