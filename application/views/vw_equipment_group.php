  ...
  <div class="row">
    <div class="col-lg-4">
      <div class="ibox-title"  style="padding-top: 10px !important;">
        <h3 class="record_form_title">Equipment Group</h3>
      </div>
      <div class="ibox-content">
        <form method="post" enctype="multipart/form-data" id="record_form">
          <div class="row" style="margin: 0px;">
            <label class="form-control col-lg-6" style="border:0px;">Equipment Group Name <a style="color: red">*</a><b class="pull-right">:</b></label>
            <div class="col-lg-6">
        ...
        <div class="table-responsive" >
          <table class="table table-striped table-bordered table-hover datatable_list" id="datatable_list" style="width: 99% !important;">
            <tfoot>
              <tr>
                <th nowrap="nowrap" ></th>
                <th nowrap="nowrap" style="width: fit-content !important;">Equipment Group name </th>
                <th nowrap="nowrap" ></th>
              </tr>
            </tfoot>
          </table>
...
<script type="text/javascript">
  ...
  function get_list(){
    ...
    var table=$('#datatable_list').DataTable({
      ...
      'stateSave': true,
      'ajax': {
        'url':'<?=base_url()?>equipment-group-list',
      },
      'columns': [
        {title: "Sr.No.",data:"sr_no",mSearch:false},
        {title: "Equipment Group Name",data:"equ_name",mSearch:true},
  ...
  function reset_form(){
    $('.record_form_title').html('Add Equipment Group Name');
    $('#equ__group_name').val('');
    $('#equ_name').val('');
    $('#equ_qty').val('');
    $('#e_id').val('');
  ...
  $('#record_form').on('submit',function(event){
    ...
    if(form.valid()==true){
      $.ajax({
        url:'<?=base_url()?>equipment-group-action',
        method:'post',
        data:formData,
        cache: false,
        contentType: false,
  ...
  $(document).on('click','.delete_record',function () {
    ...
    if (e_id) {
      if (confirm('Are Your Sure Want To Delete This Record ?')==true) {
        $.ajax({
          url:'<?=base_url()?>equipment-group-delete',
          method:'post',
          data:{e_id:e_id},
          async: false,
          success:function (data) { 
  ...
  $(document).on('click','.edit_record',function () {
   ...
   // alert(e_id);
    if (e_id) {
      $.ajax({
        url:'<?=base_url()?>equipment-group-assign-by-id',
        method:'post',
        data:{e_id:e_id},
        async: false,
        success:function (data) {
          ...
          console.log(data);
          if(typeof(data)=='object'){
            $('.record_form_title').html('Edit Equipment Group Name');
            $('#equ_group_name').val(data['record']['equ__group_name']);
