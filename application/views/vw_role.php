  ...
  <div class="row">
    <div class="col-lg-4">
      <div class="ibox-title"  style="padding-top: 10px !important;">
        <h3 class="record_form_title">Add Role</h3>
      </div>
      <div class="ibox-content">
        <form method="post" enctype="multipart/form-data" id="record_form">
          <div class="row" style="margin: 0px;">
            <label class="form-control col-lg-5" style="border:0px;">Role Name<a style="color: red">*</a><b class="pull-right">:</b></label>
            <div class="col-lg-7">
                <input class="form-control" type="text" name="r_name" id="r_name" maxlength="50" required="" >
...
<script type="text/javascript">
  ...
  function reset_form(){
    $('.record_form_title').html('Add Role');
    $('#r_name').val('');
    $('#r_id').val('');
  }
  ...
  $(document).on('click','.edit_record',function () {
    ...
    if (r_id) {
      $.ajax({
        url:'<?=base_url()?>role-by-id',
        method:'post',
        data:{r_id:r_id},
        async: false,
        success:function (data) {
          if(typeof(data)=='object'){
            $('.record_form_title').html('Edit Role');
            $('#r_name').val(data['record']['name']);
            $('#r_id').val(r_id);
          }else{
            location.reload();
          }
