  ...
  <div class="row">
    <div class="col-lg-4">
      <div class="ibox-title"  style="padding-top: 10px !important;">
        <h3 class="record_form_title">Add Specialization</h3>
      </div>
      <div class="ibox-content">
        <form method="post" enctype="multipart/form-data" id="record_form">
          <div class="row" style="margin: 0px;">
            <label class="form-control col-lg-5" style="border:0px;">Specialization Name<a style="color: red">*</a><b class="pull-right">:</b></label>
            <div class="col-lg-7">
                <input class="form-control" type="text" name="s_name" id="s_name" maxlength="50" required="" >
...
<script type="text/javascript">
  $('#specialization_tab').addClass('active');
  get_list();
  ...
  function get_list(){
    ...
    var table=$('#datatable_list').DataTable({
      ...
      'stateSave': true,
      'ajax': {
        'url':'<?=base_url()?>specialization-list',
      },
      'columns': [
        {title: "Sr.No.",data:"sr_no",mSearch:false},
        {title: "Name",data:"name",mSearch:true},
        {title: "User Count",data:"count",mSearch:false},
  ...
  function reset_form(){
    $('.record_form_title').html('Add Specialization');
    $('#s_name').val('');
    $('#s_id').val('');
  }
  ...
  $('#record_form').on('submit',function(event){
    ...
    if(form.valid()==true){
      $.ajax({
        url:'<?=base_url()?>specialization-action',
        method:'post',
        data:formData,
        cache: false,
        contentType: false,
  ...
  $(document).on('click','.edit_record',function () {
    ...
    if (s_id) {
      $.ajax({
        url:'<?=base_url()?>specialization-by-id',
        method:'post',
        data:{s_id:s_id},
        async: false,
        success:function (data) {
          if(typeof(data)=='object'){
            $('.record_form_title').html('Edit Specialization');
