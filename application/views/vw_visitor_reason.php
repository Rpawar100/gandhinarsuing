  ...
  <div class="row">
    <div class="col-lg-4">
      <div class="ibox-title"  style="padding-top: 10px !important;">
        <h3 class="record_form_title">Add Visitor Reason</h3>
      </div>
      <div class="ibox-content">
        <form method="post" enctype="multipart/form-data" id="record_form">
          <div class="row" style="margin: 0px;">
            <label class="form-control col-lg-3" style="border:0px;">Reaon<a style="color: red">*</a><b class="pull-right">:</b></label>
            <div class="col-lg-9">
                <input class="form-control" type="text" name="vr_name" id="vr_name" maxlength="50" required="" >
                <input class="form-control" type="hidden" name="vr_id" id="vr_id">
            </div>
          </div><hr>
          <div class="row" style="margin: 0px;">
...
<script type="text/javascript">
  $('#visitor_reason_tab').addClass('active');
  get_list();
  ...
  function get_list(){
    ...
    var table=$('#datatable_list').DataTable({
      ...
      'stateSave': true,
      'ajax': {
        'url':'<?=base_url()?>visitor-reason-list',
      },
      'columns': [
        {title: "Sr.No.",data:"sr_no",mSearch:false},
        {title: "Reason",data:"name",mSearch:true},
        {title: "Visitor Count",data:"count",mSearch:false},
  ...
  function reset_form(){
    $('.record_form_title').html('Add Visitor Reason');
    $('#vr_name').val('');
    $('#vr_id').val('');
  }
  ...
  $('#record_form').on('submit',function(event){
    ...
    if(form.valid()==true){
      $.ajax({
        url:'<?=base_url()?>visitor-reason-action',
        method:'post',
        data:formData,
        cache: false,
        contentType: false,
  ...
  $(document).on('click','.edit_record',function () {
    ...
    if (vr_id) {
      $.ajax({
        url:'<?=base_url()?>visitor-reason-by-id',
        method:'post',
        data:{vr_id:vr_id},
        async: false,
        success:function (data) {
          if(typeof(data)=='object'){
            $('.record_form_title').html('Edit Visitor Reason');
            $('#vr_name').val(data['record']['name']);
            $('#vr_id').val(vr_id);
          }else{
            location.reload();
          }
